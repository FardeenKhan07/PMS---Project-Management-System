...
<!-- Update Project Modal (UPDATED: IT project types) -->
...
// fetch project data
$query_proj = mysqli_query($conn,"SELECT *,CONCAT(lastname,', ',firstname,' ',midname) as name,projects.io as status FROM projects LEFT JOIN project_team ON projects.tid = project_team.tid LEFT JOIN employee ON project_team.eid = employee.eid WHERE project_id = '".mysqli_real_escape_string($conn,$id)."' ");
$prow = mysqli_fetch_assoc($query_proj);

// define IT-related project types (easy to edit)
$project_types = [
...
// ensure stored value is integer-ish
$selected_type = isset($prow['proposed_project']) ? (int)$prow['proposed_project'] : 0;
?>
    <div class="form-horizontal">
        ...
        <div class="form-group" id="form-login">
         ...
         <div class="form-group" id="form-login">
          ...
          <div class="col-sm-8">
           <select class="form-control input-sm" style="text-transform:capitalize" autocomplete="off" name="p_type" id="p_type" onchange="div_field()" required>
             <option value="">-- Select Project Type --</option>
             <?php
             // output options from the project_types array
             foreach($project_types as $key => $label){
                 $sel = ($key === $selected_type) ? ' selected' : '';
      ...
      <div class="form-group" id="form-login">
          ...
          <div class="col-sm-8">
            <input class="form-control" style="text-transform:capitalize" id="" name="pname" type="text" value="<?php echo htmlspecialchars($prow['project'] ?? '') ?>"  required>
          </div>
        </div>
      ...
      <div class="form-group" id="form-login">
          ...
          <div class="col-sm-8">
            <textarea class="form-control" style="text-transform:capitalize" id="" name="location" type="text"   required><?php echo htmlspecialchars($prow['location'] ?? '') ?></textarea>
          </div>
        </div> 
      ...
      <div class="form-group" id="form-login">
          ...
          <div class="col-sm-6">
            <input class="form-control" style="text-align:right" id="cc" name="cost" type="text" placeholder="Php." value="<?php echo htmlspecialchars($prow['overall_cost'] ?? '') ?>">
          </div>
        </div>
      ...
      <div class="form-group" id="form-login">
          ...
          <div class="col-sm-8">
            <input class="form-control"  id="" name="sdate" type="date"  value="<?php echo htmlspecialchars($prow['start_date'] ?? '') ?>" required>
          </div>
        </div>
        ...
        <div class="form-group" id="form-login">
          ...
          <div class="col-sm-8">
            <input class="form-control"  id="" name="deadline" type="date"  value="<?php echo htmlspecialchars($prow['deadline'] ?? '') ?>" required>
          </div>
        </div>
        ...
        <div class="form-group" id="form-login">
          ...
          <div class="col-sm-8">
            ...
            <option value="<?php echo htmlspecialchars($prow['tid'] ?? '') ?>"> <?php echo htmlspecialchars($prow['name'] ?? '') ?></option>
            <?php
              $pos_query = mysqli_query($conn,"SELECT *,Concat(lastname,', ',firstname,' ',midname) as name FROM project_team LEFT JOIN employee ON project_team.eid = employee.eid WHERE project_team.eid != '".mysqli_real_escape_string($conn, $prow['eid'] ?? '')."' ORDER BY name ASC");
              while($pos_row = mysqli_fetch_assoc($pos_query)){
             ?>
            <option style="text-transform:capitalize" value="<?php echo htmlspecialchars($pos_row['tid']) ?>"><?php echo htmlspecialchars($pos_row['name']) ?></option>
            <?php } ?>
            </select>
          </div>
        </div>
          ...
          <div class="col-sm-8">
            ...
            <option value="<?php echo htmlspecialchars($prow['status'] ?? '') ?>">
              <?php 
                if(($prow['status'] ?? '') == '1'){
                  echo 'On Going';
